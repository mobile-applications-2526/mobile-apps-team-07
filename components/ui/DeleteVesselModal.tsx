...
export function DeleteVesselModal({
  ...
  return (
    ...
    >
      <View testID="delete-vessel-modal" className="flex-1 bg-black/50 items-center justify-center px-6">
        <View testID="confirm-delete-dialog" className="bg-white dark:bg-[#2c2c2e] rounded-2xl w-full max-w-sm overflow-hidden">
          {/* Header with Icon */}
          <View className="items-center pt-6 pb-4 px-6">
            <View testID="delete-vessel-icon" className="w-14 h-14 rounded-full bg-red-100 dark:bg-red-900/30 items-center justify-center mb-4">
              <AlertTriangle size={28} color="#ef4444" />
            </View>
            <ThemedText testID="delete-vessel-title" type="defaultSemiBold" className="text-lg text-center">
              Delete {vesselName}?
            </ThemedText>
          </View>
          ...
          <View className="px-6 pb-6">
            {hasError ? (
              <ThemedText testID="delete-error-message" className="text-sm text-center text-gray-600 dark:text-gray-400">
                Failed to delete vessel. Please check your connection and try again.
              </ThemedText>
            ) : (
              <>
                <ThemedText testID="delete-vessel-message" className="text-sm text-center text-gray-600 dark:text-gray-400">
                  This will permanently remove all associated voyages, noon reports, and documents.
                </ThemedText>
                {hasActiveVoyage && (
                  <View testID="active-voyage-warning" className="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
